...
double speedLeftCircle[4] = {-0.25 * v, -0.25 * v, 0.25 * v, 0.25 * v};
double speedRightCircle[4] = {0.25 * v, 0.25 * v, -0.25 * v, -0.25 * v};
...
#define sampleNum 2000 // 采样点个数

const double dstX = -1.9;
const double dstY = -2.4;
  ...
  while (robot->step(timeStep) != -1)
  {
      ...
      for (size_t i = outlierCnt; i < lidarRes - outlierCnt; ++i)
      {
        if (isfinite(lidarImage[i]) && lidarImage[i] != 0)
        {
          ...
          if (imgX >= 0 && imgX < map.cols && imgY >= 0 && imgY < map.rows)
          {
            ...
            circle(map, Point(imgX, imgY), 18, 0, -1);
          }
        }
      }
    ...
    // {
      ...
      if (path.empty() || needPlan(map, path, car_pos))
      {
        ...
        while (path.empty() || (path[path.size() - 1].x != dst_pos.x && path[path.size() - 1].y != dst_pos.y));
      }
      // if (path.empty() || needPlan(map, path, car_pos))
      // {     
      //   path = prm(map, car_pos, dst_pos);
